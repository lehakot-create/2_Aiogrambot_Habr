...
from models.models import User, Themes
...
class DBManager:
    ...
    def add_user(self, user_telegram_id: int,
                 username: str,
                 first_name: str,
                 full_name: str):
        """
        ...
        """
        result = self.get_user(user_telegram_id=user_telegram_id)
        if not result:
            new_user = User(user_telegram_id=user_telegram_id,
                            username=username,
    ...
    def get_user(self, user_telegram_id: int):
        """
        Возвращает пользователя по его telegam id
        """
